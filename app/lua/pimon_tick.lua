...
--   -- df output looks like: "63%". We must remove the percent sign.
     local out = utils.run_cmd([[df -P /hostfs | awk 'NR==2{print $5}']]) or ""
       out = out:gsub("%s+", "")
         out = out:gsub("%%", "")       -- strip literal percent sign
           local n = tonumber(out)
             return n or 0
             end

local function cpu_temp_c()
  local t = utils.read_file("/sys/class/thermal/thermal_zone0/temp")
  if not t then
    return nil
  end
  local milli = tonumber(t)
  if not milli then
    return nil
  end
  return milli / 1000.0
